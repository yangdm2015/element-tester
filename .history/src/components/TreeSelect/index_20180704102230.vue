    ...
    export default {
        ...
        methods: {
            isNodeDisabled(data, node) {
                return !!this.disabledKeys && !!this.disabledKeys.length
                        && (this.disabledKeys.includes(data.id)
                        || data.children && data.children.length && data.children.every(childData => this.disabledKeys.includes(childData.id)));
            },
            focusInput() {
            ...
            deleteTag(e, data) {
                ...
                let selectedNodes = this.$refs.tree.getCheckedNodes().filter((node)=> this.isSelectLevel(node));//这里拿到的就是取消过后的数据了。非选择顺序
            ...
            updateSelectFromInput() {
                ...
                if (this.multiple) {
                    ...
                    let selectedNodes = this.$refs.tree.getCheckedNodes().filter((node)=> this.isSelectLevel(node));//非选择顺序
                    //为了保证选择顺序。新增和减少都可以满足
                    this.selectedNodes = [...new Set([...this.selectedNodes.filter((node)=> selectedNodes.includes(node)), ...selectedNodes])];
                }
                this.keyword = '';
            ...
            removeNodeFromSelect(data) {
                if (this.isSelectLevel(this.$refs.tree.getNode(data))) {
                    this.selectedNodes = this.selectedNodes.filter(nodeData=> nodeData.id !== data.id);
                }
            },
            getNodesInSelectLevel(data) {
                ...
                let levels = Array.isArray(this.selectLevel) ? this.selectLevel.map(l => +l) : [+this.selectLevel];
            ...
            handleCheck(node, checkedStatus) {
                ...
                let {checkedNodes, checkedKeys, halfCheckedNodes, halfCheckedKeys} = checkedStatus;
                let selectedNodes = checkedNodes.filter((node)=> this.isSelectLevel(node));
            ...
            filterNode(value, data) {
                ...
                for (let i = node.level; i > 0; i--) {
                    if (node.data[this.props.label].indexOf(value) > -1) return true;
                    node = node.parent;
                }
                return false;
            },
